...
Polygon halfplane_intersection (Polygon & p,Line & line)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        double c2=cross(p1,p2,target);
        double d=cross(p1,p2,p[(i+1)%n]);
            if(c*c2>=0)q.push_back(p[i]);
        if(c*d<0) q.push_back(GetLineIntersection(p1,p2-p1,p[i],p[(i+1)%n]-p[i]));
    }
    return q;
}
double solve(Polygon & p,vector<Line> &lines)
{
...
int main()
{
	...
	while(cin>>N>>W>>H>>x>>y)
	{
        ...
        in.push_back(Point(0,H));
        in.push_back(Point(W,H));
        in.push_back(Point(W,0));
        vector<Line> lines;
        for(int i=1;i<=N;i++)
        {
            double a1,a2,b1,b2;
            ...
            Point p2(b1,b2);
            Line tmp;
//            if(cross(p1,p2,target)<0) swap(p1,p2);
            tmp.p1=p1;
            tmp.p2=p2;

            lines.push_back(tmp);
        }
		    printf("Case #%d: %.3lf\n",++times,solve(in,lines)+EPS);
	}
    return 0;
}
