/*************************************************************************
	...
	> Created Time: 2016/06/14 19:35:36
 ************************************************************************/
...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            cin>>x>>y>>w>>h>>phi;
            Vector v[4];
            v[0]=Vector(w/2.0,h/2.0);
            v[1]=Vector(-w/2.0,h/2.0);
            v[2]=Vector(w/2.0,-h/2.0);
            v[3]=Vector(-w/2.0,-h/2.0);
            for(int j=0;j<4;j++)
            {
                P.push_back(Point(x,y)+Rotate(v[j],(-1.0)*phi/180*acos(-1.0)));        
            }
            allarea+=w*h;
        }
        Point ch[10000];
        ...
        //cout<<cnt<<endl;
        double parea=PolygonArea(ch,cnt);
        double percent=allarea/parea;
        //cout<<allarea<<" "<<parea<<endl;
        percent*=100.0;
        printf("%.1lf %%\n",percent);
